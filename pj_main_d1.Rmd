...
According to SAT/ACT scores, we would like to know whether liberal states perform better or worse than conservative states, and which ethnicity perform best
...
college = college %>%
  mutate_at(vars("SAT_AVG", "SATVRMID", "SATMTMID", "ACTENMID", "ACTMTMID", "UGDS_WHITE", "UGDS_BLACK", "UGDS_HISP", "UGDS_ASIAN", "UGDS_AIAN", "UGDS_NHPI", "UGDS_2MOR", "UGDS_NRA", "UGDS"), as.numeric)
...
viz = file %>%
  select("INSTNM", "STABBR", "SATVRMID", "SATMTMID", "ACTENMID", "ACTMTMID", "UGDS_WHITE", "UGDS_BLACK", "UGDS_HISP", "UGDS_ASIAN", "UGDS_AIAN", "UGDS_NHPI","UGDS_2MOR","UGDS_NRA", "ZIP", "LATITUDE", "LONGITUDE", "CITY")
...
uniInfo = paste(viz[['INSTNM']], "<br>", 
                ...
                "Avg ACT Math", viz[['ACTMTMID']], "<br>", 
                "Avg ACT Eng", viz[['ACTENMID']], "<br>", 
...
viz = filter(viz,
              ...
              is.na(ACTENMID) == FALSE &
              is.na(ACTMTMID) == FALSE)
...
```{r average ACTs score for each subjects, echo=FALSE}
act = college %>%
...
- The density of ACT Math score is high at around 22

- Achieving perfect ACT Math is more rare
...
```{r school with top ACT Math, echo=FALSE}
...
ggplot(top_act_math, aes(x=reorder(INSTNM, mean_act_m), y=mean_act_m)) +
  ...
  coord_flip() +
  ggtitle("Institution with highest average ACT Math score") +
  xlab("Institution") +
  ylab("Average ACT Math Score")
```

```{r school with top ACT Eng, echo=FALSE}
...
ggplot(top_act_eng, aes(x=reorder(INSTNM, mean_act_e), y=mean_act_e)) +
  ...
  coord_flip() +
  ggtitle("Institution with highest average ACT English Score") +
  xlab("Institution") +
  ylab("Average ACT English Score")
```
...
score %>% 
  ...
  labs(title = "Residual of predicted avg sat score")

```
Hypothesis testing on TODO
